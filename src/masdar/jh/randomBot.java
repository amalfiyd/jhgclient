...
public class randomBot
{
	...
	private static JhBotApi api = new JhBotApi("http://www.juniorhighgame.com", "114397137729536594852-1417587405217");
	...
	private static HashMap<String, Integer> makeMove(JSONObject state) throws Exception
	{
		...
		transactionsToDo = randomMove(users, availableTokens);
		
		giveHistory.add(transactionsToDo);
		return transactionsToDo;
	}
			
	private static HashMap<String, Integer> randomMove(JSONArray users, int availableTokens) throws Exception 
	{
		...
		for (int i = 0; i < users.length(); i++)
		{
			...
			if (!user.optBoolean("isCurrentPlayer"))
			{
				...
				if (randomNum <= availableTokens)
				{
					transactionsToDo.put(user.optString("id"), Integer.valueOf(randomNum));
					availableTokens -= Math.abs(randomNum);
				}
			}
		}
